"""Focused 588200 moving-average crossover strategy search."""
...
from src.strategies.etf_588200 import (  # noqa: E402
    ma_crossover_signal_for as signal_for,
    ma_crossover_spec_from_row as spec_from_row,
    make_ma_crossover_specs as make_specs,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="588200 MA crossover strategy search.")
    ...
    parser.add_argument(
        "--output-dir",
        default=str(PROJECT_ROOT / "outputs" / "588200_ma_crossover_strategy"),
    )
...
def write_report(
    ...
    if not trade_table.empty:
        for col in ["entry_signal_close", "entry_open", "exit_signal_close", "exit_open"]:
            trade_table[col] = trade_table[col].map(lambda x: f"{x:.4f}")
        for col in ["gross_return", "net_approx"]:
            trade_table[col] = trade_table[col].map(lambda x: pct(x))
    trade_md = markdown_table(trade_table.tail(20)) if not trade_table.empty else "无已闭合交易。"
    text = f"""# 588200 均线交叉动量策略搜索报告
...
- 均线窗口：5MA、20MA、60MA、120MA，两两组成快慢均线交叉。
...
- 选择口径：本报告的“最优”按 588200 样本外夏普排序；同时列出训练池/样本外池指标，提醒过拟合风险。

## 最优策略

策略：`{best_spec.name}`

- 快均线：{best_spec.fast_window} 日
- 慢均线：{best_spec.slow_window} 日
- 成分股广度过滤：{"未启用" if best_spec.breadth_buy is None else f"MA60 广度买入 {best_spec.breadth_buy:.0%} / 卖出 {best_spec.breadth_sell:.0%}"}
- 波动率过滤：{"未启用" if best_spec.vol_window is None else f"{best_spec.vol_window} 日波动率分位 <= {best_spec.vol_max:.0%}"}
...
1. {best_spec.fast_window} 日均线大于 {best_spec.slow_window} 日均线。
2. 如果启用成分股广度过滤，则成分股池 MA60 广度不低于买入阈值。
3. 如果启用波动率过滤，则 588200 的指定窗口波动率分位不高于上限。
4. 如果启用 `poolmom`，则成分股池 20 日收益中位数必须大于 0。

## 卖点规则

持仓后，任一条件触发时，下一交易日开盘卖出或继续空仓：

1. {best_spec.fast_window} 日均线小于等于 {best_spec.slow_window} 日均线。
2. 如果启用成分股广度过滤，则成分股池 MA60 广度跌到卖出阈值或更低。
3. 如果启用波动率过滤，则 588200 波动率分位高于卖出上限。
4. 如果启用 `poolmom`，则成分股池 20 日收益中位数小于等于 0。
...
下面列的是信号日和下一交易日开盘执行价，不再把信号日收盘价当成执行价。
...
- 训练池第一和 588200 样本外第一可能不同。本轮稳健候选为：`{robust_name}`。
- 如果某策略训练池表现弱、但 588200 样本外特别强，应标记为“事后最优候选”，不能直接当作未来最优。
...
def main() -> None:
    ...
    run_dir = Path(args.run_dir) if args.run_dir else Path(args.output_dir) / f"ma_cross_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for i, spec in enumerate(specs, start=1):
        ...
        rows.append(row)
        if args.checkpoint_every > 0 and (i % args.checkpoint_every == 0 or i == len(specs)):
    ...
    robust = robust_pool.sort_values(["target_test_sharpe", "pool_test_median_sharpe"], ascending=False).iloc[0] if not robust_pool.empty else pd.Series(dtype=object)
    ...
    print("\nMA crossover search completed", flush=True)
